...
public:
   ...
   /// @brief Check if ID matches that passed in
    ...
    virtual bool idMatches(const char* pDeviceId) const override final
    {
        ...
        if (addrStr.indexOf('x')){
            addrStr = addrStr.substring(addrStr.indexOf('x') + 1);
        }
        String mux = "";
        ...
        LOG_I("RaftBusDevice", "idMatches busNameStr %s busName %s addrStr %s address %s", busNameStr.c_str(), _busName.c_str(), (mux + addrStr).c_str(), String(_address, 16).c_str());
        if (busNameStr != _busName)
            return false;
        return String(_address, 16) == mux + addrStr;
    }
